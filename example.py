...
for idx,d in enumerate(data):
    idx+=1 
    canvas = iifpapi3.Canvas()
    canvas.set_id(extendbase_url=["canvas","p%s"%idx]) # in this case we use the base url
    canvas.set_height(d[2])
    canvas.set_width(d[1])
    canvas.add_label("en",d[0])
    annopage = iifpapi3.AnnotationPage()
    annopage.set_id(extendbase_url=["page","p%s"%idx,"1"])
    annotation = iifpapi3.Annotation(target=canvas.id)
    annotation.set_id(extendbase_url=["annotation","p%s-image"%str(idx).zfill(4)])
    annotation.set_motivation("painting")
    annotation.body.set_id("".join(d[3:]))
    annotation.body.set_type("Image")
    annotation.body.set_format("image/jpeg")
    annotation.body.set_width(3204)
    annotation.body.set_height(4613)
    annotation.body.profile = None # we mute the suggested warning
    s = iifpapi3.service()
    s.set_id(d[3])
...
manifest.json_save("manifes.json")